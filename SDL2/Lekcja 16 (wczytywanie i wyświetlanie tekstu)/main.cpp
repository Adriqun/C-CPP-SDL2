...
std::string loadFromNotebook(std::string path)
{
    std::fstream file;
    std::string inscription;
    file.open(path.c_str());
    getline(file,inscription);
    file.close();
    return inscription;
}
bool init()
...
bool loadMedia()
{
    ...
    else
    {
        ...
        textTexture=loadFromRenderedText( loadFromNotebook("tekst.txt"), textColor );
        if( textTexture==NULL )
        {
